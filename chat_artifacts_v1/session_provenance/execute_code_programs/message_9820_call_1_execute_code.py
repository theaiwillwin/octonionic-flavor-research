...
try:
    ...
    env.pop("PYTHONPATH", None)
    completed = subprocess.run(
        [r"D:/Projects/toe_new/.venv/Scripts/python.exe", str(path)],
        cwd=r"D:/Projects/ToE_21st_June_NEWEST", env=env,
        text=True, capture_output=True, timeout=300,
    )
    print("TEMP_VERIFIER=" + str(path))
    print(completed.stdout, end="")
    if completed.stderr:
        print(completed.stderr, end="")
    if completed.returncode != 0:
        raise RuntimeError(f"ad-hoc verifier exited {completed.returncode}")
finally:
    if path is not None and path.exists():
        path.unlink()
    print("TEMP_VERIFIER_CLEANED=" + str(path is not None and not path.exists()))
