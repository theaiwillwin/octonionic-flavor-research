"""Generate publication figures from locked target-free pipeline artifacts."""
...
OUT = ROOT / "output" / "pdf" / "target_free_g2_arxiv_v1_figures"
SHAPE = ROOT / "target_free_g2_flavor_shape_verifier_v1_results.json"


def main():
    OUT.mkdir(parents=True, exist_ok=True)
    targets = (OUT / "selection_funnel.png", OUT / "isolated_full_rank_spectra.png")
    ...
    labels = [
        "Locked actions",
        "Reliable stable vacua",
        "Isolated modulo symmetry",
        "Isolated with selected $V$",
        "Isolated, full rank",
        "Two-gap hierarchy",
    ]
    ...
    for ax, row in zip(axes.flat, rows):
        ...
        ax.set_title(row["label"].replace("rademacher_", "$c^{(") + ("}$" if row["label"].startswith("rademacher_") else ""), fontsize=9)
    ...
    fig.savefig(targets[1], bbox_inches="tight")
    plt.close(fig)

    print(json.dumps({"outputs": [str(x) for x in targets], "spectra_panels": len(rows)}, indent=2))
