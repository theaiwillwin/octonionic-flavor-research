...
RESULTS = ROOT / "test_pmns_polar_link_reachability_v1_results.json"
REPORT = ROOT / "PMNS_POLAR_LINK_REACHABILITY_REPORT_v1.md"
...
def construct_complex_basis(j_h: np.ndarray, h: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Construct p_a,q_a with q_a=-J_h p_a and p_a^T J_h q_b=delta_ab."""
...
def main() -> int:
    ...
    result = {
        "schema": "pmns-polar-link-reachability/v1",
        "status": "PASS_EXACT_CONSTRUCTIVE_REACHABILITY" if passed else "FAIL",
        "claim": (
            "The original polar-link map is surjective onto U(3) when the real "
            "frames are allowed to vary freely. PMNS is therefore geometrically "
        ...
        "construction": {
            ...
            "L_nu": l_nu.tolist(),
            "identity": (
                "Choose an orthonormal complex basis p_a,q_a=-J_h p_a. "
    ...
    report = f"""# PMNS polar-link reachability gate v1
