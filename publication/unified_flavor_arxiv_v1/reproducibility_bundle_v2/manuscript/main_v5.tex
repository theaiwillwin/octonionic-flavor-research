...
\hypersetup{
  ...
  pdftitle={Reachability Is Not Selection in Target-Free G2 Flavor-Frame Dynamics},
...
\title{\textbf{Reachability Is Not Selection in Target-Free $G_2$ Flavor-Frame Dynamics}\\[0.35em]
\large Finite vacuum, hierarchy, and lepton-link gates}
...
These are negative finite gates, not a global no-go theorem.  They localize the missing structure: scalar closed-overlap invariants and a singular-value-only link do not provide a nondegenerate family-axis selector.  The geometry can represent any unitary mixing matrix, but the tested dynamics does not select one.
\end{abstract}
...
No result below establishes a Standard Model derivation, a global classification of the invariant ring, or a proof that unobserved stationary branches do not exist.
...
For each action, four generic starts were optimized on the Stiefel product using standard matrix-manifold retractions~\cite{AbsilMahonySepulchre2008}.  A branch was called stationary when the projected gradient norm was below $10^{-5}$.  The 48-dimensional local Hessian was classified using thresholds
\begin{equation}
...
\subsection{Exact reachability does not imply dynamical selection}

The overlap geometry is not too small to represent realistic mixing.  Choose an orthonormal Lagrangian frame $P=(p_1,p_2,p_3)\subset h^\perp$ satisfying
\begin{equation}
 P^{\mathsf T}J_hP=0,
\end{equation}
and set $Q=-J_hP$.  The six columns of $(P,Q)$ are orthonormal.  For any target $U\in U(3)$, define
\begin{equation}
 L_e=P,\qquad L_\nu=P\,\Re U+Q\,\Im U.
 \label{eq:unitary-reachability}
\end{equation}
Then $L_e$ and $L_\nu$ are real orthonormal frames and direct substitution gives
\begin{equation}
 L_e^{\mathsf T}(P_h+iJ_h)L_\nu=U.
 \label{eq:unitary-surjection}
\end{equation}
Thus the polar-link map is surjective onto $U(3)$.  A stress test over 1000 seeded Haar-random unitaries gave a worst polar residual of $1.34\times10^{-15}$.  This is an analytic representability statement with a numerical implementation check, not a vacuum-selection result.  Indeed, on the unitary image of \cref{eq:unitary-surjection}, the nuclear norm is the constant $\|U\|_*=3$ and cannot distinguish orientations.
...
\begin{equation}
  ...
  \qquad
  \mu_K=2.1768243,\quad \sigma_K=0.2519311.
  \label{eq:vtotal}
\end{equation}
...
\begin{quote}
\status{Negative finite gate with independently verified Hessian spectra.} Among the 16 smooth angle-evaluable branches found by the four-start 74-action solve, no quotient-stable branch lies inside all three NuFIT~6.0 three-sigma angle intervals.  Five additional rank-deficient branches are nonsmooth and are not assigned classical stability.
\end{quote}
...
\begin{enumerate}[leftmargin=2.1em]
  ...
  \item \textbf{No ultraviolet completion.}  Connecting this local four-plane model to an exceptional gauge theory such as $E_6$ would require an explicit representation-level map and a derived neutral-fermion operator; neither is supplied here.
\end{enumerate}
...
\begin{itemize}[leftmargin=2.1em]
  ...
  \item exact polar-link reachability construction and 1000-case unitary stress test;
...
\end{itemize}
The claim ledger records the exact boundary and source hashes of each finite gate.
