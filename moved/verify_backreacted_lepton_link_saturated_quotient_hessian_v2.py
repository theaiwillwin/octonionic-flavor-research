...
OUTPUT_JSON = ROOT / "verify_backreacted_lepton_link_saturated_quotient_hessian_v2_results.json"
OUTPUT_MD = ROOT / "VERIFY_BACKREACTED_LEPTON_LINK_SATURATED_QUOTIENT_HESSIAN_v2.md"
...
def main() -> int:
    ...
    assertions = {
        ...
        "all_orbit_ranks_match_gate": all(
            row["orbit_rank"]
            == next(
                gate_row["residual_su3_orbit_rank"]
                for gate_row in gate["results"]
                if gate_row["label"] == row["label"]
            )
            for row in checks
        ),
    ...
    result = {
        "schema": "verify-backreacted-lepton-link-saturated-quotient-hessian/v2",
    ...
    OUTPUT_MD.write_text(
        "\n".join(
            [
                "# Independent verification: saturated quotient Hessian v2",
                "",
